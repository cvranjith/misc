DECLARE
function fn_check_gl_vdbal (p_brn in sttms_branch.branch_code%type,
...
exception when others then
	--dbg('Failed'||sqlerrm);
	return false;
end fn_check_gl_vdbal;--ST40R2
begin
--for i in (select branch_code brn from sttm_branch where record_stat = 'O') loop
if not fn_check_gl_vdbal(i.brn,'VND')
then null;
end if;
end loop;
end;
