...
BEGIN
	...
	l_filename1 := 'PACKAGEBODIES.sql';
	pr_read_text_file(l_dir,l_filename1,tb_v);
	L_DIR := 'F:\CV\IFACE\DBSCRIPTS\PACKAGE_BODY';
	FOR N IN TB_V.FIRST .. TB_V.LAST
	LOOP
		filebuf := TB_V(N);
		IF SUBSTR(FILEBUF,1,16) = '-- Package body ' THEN
			l_filename2 := SUBSTR(FILEBUF,17)||'.SQL';
		...
		IF (	SUBSTR(FILEBUF,1,27) = '-- Start of DDL script for '
		   OR	SUBSTR(FILEBUF,1,24) = '-- Generated 22-feb-02  '
		   OR	FILEBUF = '-- from fcisuat-FCCLIVE:2'
		   OR	NVL(FILEBUF,'#$') = '#$'
