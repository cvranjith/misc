...
BEGIN
	...
	l_filename1 := 'INDEXES.sql';
	pr_read_text_file(l_dir,l_filename1,tb_v);
	L_DIR := 'F:\CV\IFACE\DBSCRIPTS\INDEX';
	FOR N IN TB_V.FIRST .. TB_V.LAST
	LOOP
		filebuf := TB_V(N);
		IF SUBSTR(FILEBUF,1,9) = '-- Index ' THEN
			l_filename2 := SUBSTR(FILEBUF,10)||'.DDL';
		...
		IF (	SUBSTR(FILEBUF,1,27) = '-- Start of DDL script for '
		   ...
		   OR   SUBSTR(FILEBUF,1,13) = '  PCTFREE    '
		   OR   SUBSTR(FILEBUF,1,13) = '  INITRANS   '
		   OR   SUBSTR(FILEBUF,1,13) = '  MAXTRANS   '
		   OR   SUBSTR(FILEBUF,1,13) = '  TABLESPACE '
		   )
		THEN
			NULL;
		ELSE
