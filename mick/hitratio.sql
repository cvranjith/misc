...
select to_number(substr(value,1,3)) into spsize from v$parameter where name = 'shared_pool_size';
select value into blkbuf  from v$parameter where name = 'db_block_buffers';
select round(value/1024/1024,2) into logbuf  from v$parameter where name = 'log_buffer';
...
dbms_output.put_line('>                    Log Buffer  = '||logbuf||' MB');
dbms_output.put_line('>');
dbms_output.put_line('> ');
if
...
END IF;
if
	rowcac < 95  then dbms_output.put_line('*** HINT: Row Cache too low! Increase the Shared Pool Size.');
END IF;
if
	bufcac < 95  then dbms_output.put_line('*** HINT: Buffer Cache too low! Increase the DB Block Buffer value.');
END IF;
if
